...
class BiSO100Leader(Teleoperator):
    ...
    def _get_left_calibration(self):
        ...
        left_calibration_path = Path("/home/zhiyuan/.cache/huggingface/lerobot/calibration/teleoperators/so100_leader") / f"{self.config.left_id}.json"
    ...
    def _get_right_calibration(self):
        ...
        right_calibration_path = Path("/home/zhiyuan/.cache/huggingface/lerobot/calibration/teleoperators/so100_leader") / f"{self.config.right_id}.json"
    ...
    def action_features(self) -> dict[str, type]:
        action_ft = {}
        for motor in self.left_bus.motors.keys():
            action_ft[f"left_{motor}.pos"] = float
        for motor in self.right_bus.motors.keys():
    ...
    def calibrate(self) -> None:
        ...
        if left_calibration is None:
            raise ValueError(f"No calibration found for left arm (ID: {self.config.left_id}). "
                           "Please ensure calibration exists at /home/zhiyuan/.cache/huggingface/lerobot/calibration/teleoperators/so100_leader/")
        
        if right_calibration is None:
            raise ValueError(f"No calibration found for right arm (ID: {self.config.right_id}). "
                           "Please ensure calibration exists at /home/zhiyuan/.cache/huggingface/lerobot/calibration/teleoperators/so100_leader/")
        ...
        logger.info(f"Successfully loaded calibrations for both arms")
