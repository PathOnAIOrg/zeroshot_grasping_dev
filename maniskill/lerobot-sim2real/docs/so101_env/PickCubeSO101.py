### edit based on  https://github.com/haosulab/ManiSkill/blob/main/mani_skill/envs/tasks/tabletop/pick_cube.py
...
from so101_arm import MySO101      # add so101 model
...
class PickCubeEnv(BaseEnv):
    ...
    agent: Union[Panda, Fetch, XArm6Robotiq, SO100, MySO101, WidowXAI]          # add so101
    cube_half_size = 0.02
    goal_thresh = 0.025
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        if self.robot_uids in ["panda", "widowxai"]:
            qvel = qvel[..., :-2]
        elif self.robot_uids == "so100" or self.robot_uids == "my_so101":
            qvel = qvel[..., :-1]
...
@register_env("PickCubeSO101-v1", max_episode_steps=50)         # NEW: env support so101
class PickCubeSO101Env(PickCubeEnv):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, robot_uids="my_so101", **kwargs)
