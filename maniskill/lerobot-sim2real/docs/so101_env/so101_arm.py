### edit from https://github.com/haosulab/ManiSkill/blob/main/mani_skill/agents/robots/so100/so_100.py
import os
import sapien
...
class MySO101(BaseAgent):
    ...
    def _after_loading_articulation(self):
        ...
        self.finger2_link = self.robot.links_map["jaw"]
        self.finger1_tip = self.robot.links_map["gripper"]
        self.finger2_tip = self.robot.links_map["jaw"]

    @property
    def tcp_pos(self):
        # computes the tool center point as the mid point between the the fixed and moving jaw's tips
        return (self.finger1_tip.pose.p + self.finger2_tip.pose.p) / 2          # TODO: so101 urdf does not include finger tip joints, may need to rewrite
    ...
    def _after_loading_articulation(self):
        super()._after_loading_articulation()
        # self.set_colors()
        # print("All links:", self.robot.links_map.keys())       # this can print out 
        self.finger1_link = self.robot.links_map["gripper"]
        self.finger2_link = self.robot.links_map["jaw"]
        self.finger1_tip = self.robot.links_map["gripper"]
        self.finger2_tip = self.robot.links_map["jaw"]
