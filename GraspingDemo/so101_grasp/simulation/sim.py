...
from so101_grasp.simulation import cameras
...
class SimGrasp(Sim):
    ...
    def so101_grasp_sequence(self, grasp_joint_angles: List[float], so101_client: SO101Client) -> List[List[float]]:
        """
        Constucts a sequence of waypoints for the SO-101 robot to perform a grasp. 
        This function executes this waypoints in sim and returns the real robot joint angle waypoints.
        Args:
            grasp_joint_angles: List[float]: The joint angles to set.
            so101_client: SO101Client: The SO-101 client to use.
        Returns:
            List[List[float]]: A list of waypoints for the SO-101 robot.
        """
        jaw_closed_angle = 0  
        jaw_open_angle = 1

        so101_waypoint0 = so101_client.read_joints()
        so101_waypoint0[2] -= 0.6
        ...
        so101_waypoint2 = grasp_joint_angles[1:].copy()
        ...
        # Grasp pose with jaw closed
        so101_waypoint3 = closed_joint_angles[1:].copy()
        ...
        so101_waypoint1 = lift_obj_joint_angles[1:].copy()
        so101_waypoint1[-1] = jaw_open_angle

        # Lift gripper up with jaw closed
        so101_waypoint4 = lift_obj_joint_angles[1:].copy()
        return [so101_waypoint0, so101_waypoint1, so101_waypoint2, so101_waypoint3, so101_waypoint4]

    def so101_place_sequence(self, place_point: List[float], so101_client: SO101Client) -> List[List[float]]:
        """
        Constucts a sequence of waypoints for the SO-101 robot to perform a place. 
        This function executes this waypoints in sim and returns the real robot joint angle waypoints.
        Args:
        ...
        place_waypoint0 = so101_client.read_joints()
        return [place_waypoint0, place_waypoint1, place_waypoint2]
    
    def move_to_position(self, position: List[float], orientation: np.ndarray = None) -> bool:
        """
        Move the robot end effector to a target position with optional orientation.
        
        Args:
            position: Target position [x, y, z] in robot base frame
            orientation: Optional 3x3 rotation matrix for end effector orientation
        ...
        if orientation is None:
            current_state = pb.getLinkState(self.robot_id, gripper_link_idx)
            target_orientation = current_state[1]  # Current orientation as quaternion
        else:
            # Convert rotation matrix to quaternion
            from scipy.spatial.transform import Rotation as R
            target_orientation = R.from_matrix(orientation).as_quat()
        ...
        try:
            ...
            # Execute movement
            self._execute_joint_control(list(target_joint_angles), steps=60)
            
            # Verify we reached the target
            final_state = pb.getLinkState(self.robot_id, gripper_link_idx)
            final_pos = np.array(final_state[0])
            error = np.linalg.norm(final_pos - np.array(position))
