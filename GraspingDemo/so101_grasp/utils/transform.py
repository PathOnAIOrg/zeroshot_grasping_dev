...
def make_robot_config(robot_type : str = "so101") -> GBRobotConfig:
    """
    Make a robot config object describing the grasp offsets for a given robot.

    Args:
        robot_type (str): The type of robot. Options: "so101"
    Returns:
        GBRobotConfig: The robot config object.
    """
    if robot_type == "so101":
        return GBRobotConfig(x_grasp_shift=SO101_X_GRASP_SHIFT, y_grasp_shift=SO101_Y_GRASP_SHIFT, z_grasp_shift=SO101_Z_GRASP_SHIFT, robot_type="so101")
    else:
        raise ValueError(f"Invalid robot type: {robot_type}. Supported types: 'so101'")


# Define paths for real robot calibration
REAL_ROBOT_TRANSFORM_PATH = "config/calibration/camera_transform.npy"
REAL_ROBOT_SCALING_FACTOR_PATH = "config/calibration/scaling_factor.npy"
...
def grasp_service_to_robot_format(grasps: List[Grasp], robot_config : GBRobotConfig) -> List[Grasp]:
    ...
    Args:
        grasps (List[Grasp]): List of grasps in camera frame format.
